...
class TestLocalFolderSource:
    def test_poll_empty_directory(self, temp_dir: Path) -> None:
        source = LocalFolderSource(input_dir=temp_dir)
        requests = _poll(source)
        assert requests == []

    def test_poll_detects_pdf(self, temp_dir: Path) -> None:
    ...
    def test_poll_ignores_hidden_files(self, temp_dir: Path) -> None:
    ...
    def test_poll_ignores_directories(self, temp_dir: Path) -> None:
    ...
    def test_poll_ignores_unsupported_extensions(self, temp_dir: Path) -> None:
    ...
    def test_poll_ignores_oversized_files(self, temp_dir: Path) -> None:
        (temp_dir / "huge.pdf").write_bytes(b"%PDF-1.4" + b"x" * 1000)
        source = LocalFolderSource(input_dir=temp_dir, max_file_size=10)
        requests = _poll(source)
        assert len(requests) == 0

    def test_poll_ignores_empty_files(self, temp_dir: Path) -> None:
        (temp_dir / "empty.pdf").write_bytes(b"")
        source = LocalFolderSource(input_dir=temp_dir)
        requests = _poll(source)
        assert len(requests) == 0

    def test_poll_nonexistent_directory(self) -> None:
        source = LocalFolderSource(input_dir="/nonexistent/path")
        requests = _poll(source)
        assert requests == []

    def test_poll_multiple_files(self, temp_dir: Path) -> None:
